...
function App() {
  ...
  // Seat switch from inside a workspace — re-seeds the lane so Director lands
  // in the contextual edit bay and Producer/Client in the business frame.
  function onSeat(l) {
    setLane(l);
    setCat(FRAMEWORK.top10(l)[0].id);
    setSubIdx(0);
  ...
  return (
    <div className="stage" id="stage">
      ...
      {screen === 'work' && lane === 'director' && (
        <ContextualEditBay state={state} cat={cat} subIdx={subIdx} flashId={flashId}
          dirChat={dirChat} onCat={onCat} onSub={onSub} onMode={onMode} onSendDir={onSendDir}
          seat={lane} onSeat={onSeat} onReset={reset} />
      )}
      {screen === 'work' && lane !== 'director' && (
        <ContextualFrame seat={lane} onSeat={onSeat} onReset={reset} />
      )}
    </div>
  );
}
